...
bool AppChap1_6::Init(const char* vCmdLine[], const int iCmdCount, const int iWidth, const int iHeight){
    ...
    }
    {
        ...
        auto V = NX::LookAt(NX::float3(0, 0, -200), NX::float3(0, 0, 1), NX::float3(0,1,0));
        auto M = NX::Translate<float>(0, 0, 0);
        MVP = P * V * M;
    }
...
void AppChap1_6::Render(){
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo);
    auto P = NX::Perspective<float>(90, 1, 1, 10000);
    auto V = NX::LookAt(NX::float3(0, -10, -100), NX::float3(0, 0, 1), NX::float3(0,1,0));
    auto M = NX::Translate<float>(0, 0, 0);
