#include "AppChap1_7.h"

AppChap1_7::AppChap1_7(){
    Radius = 0;
}
...
bool AppChap1_7::Init(const char* vCmdLine[], const int iCmdCount, const int iWidth, const int iHeight){
    ...
    {//program
        m_pg = new NX::Program();
        m_pg->AddShader("./Chap1/VS1_7.glsl", GL_VERTEX_SHADER);
        m_pg->AddShader("./Chap1/FS1_7.glsl", GL_FRAGMENT_SHADER);
        m_pg->LinkProgram();
    }
    {//vertex
        memset(v, 0, sizeof(v));
    ...
    {//texture
        ...
        tex = m_pt->Load("image/texture.jpg");
        glBindTexture(GL_TEXTURE_2D, tex);
...
void AppChap1_7::Tick(const double DeltaTime){
    Application::Tick(DeltaTime);
    Radius += DeltaTime / 5;
}

void AppChap1_7::Render(){
...
void AppChap1_7::OnKeyEvent(int key, int scancode, int action, int mods){
